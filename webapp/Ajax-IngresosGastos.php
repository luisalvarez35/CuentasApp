...
$sql = "SELECT id, tipo, idUsuario, idCategoria, idCuenta, fecha, importe ";
$sql.=" FROM ingresosGastos ";
$sql.=" WHERE idUsuario = '".$_SESSION['id']."%' ";
$query = mysqli_query($mysqli, $sql) or die("Ajax-IngresosGastos.php: obtiene los datos para el grid.");
...
if ( !empty($requestData['search']['value']) )
{
    ...
    $sql.=" FROM ingresosGastos";
    $sql.=" WHERE idUsuario = '".$_SESSION['id']."%' ";
    $sql.=" AND (id LIKE '".$requestData['search']['value']."%' ";    // $requestData['search']['value'] contiene parametros de busqueda
    $sql.=" OR tipo LIKE '".$requestData['search']['value']."%' ";
    $sql.=" OR idCategoria LIKE '".$requestData['search']['value']."%' ";
    $sql.=" OR idCuenta LIKE '".$requestData['search']['value']."%' ";
    $sql.=" OR fecha LIKE '".$requestData['search']['value']."%' ";
    $sql.=" OR importe LIKE '".$requestData['search']['value']."%' )";
    $query=mysqli_query($mysqli, $sql) or die("Ajax-IngresosGastos.php: get PO");
    ...
    $query=mysqli_query($mysqli, $sql) or die("Ajax-IngresosGastos.php: get PO"); // Ejecutamos la consulta otra vez con los límites indicados
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
...
///
/// Suma Balance
        //$sql=" SELECT SUM(importe) FROM ingresosGastos  WHERE idUsuario = '".$_SESSION['id']."%' ";
       //$query = mysqli_query($mysqli, $sql) or die("Ajax-IngresosGastos.php: obtiene los datos para el grid.");
        //$foot = mysqli_num_rows($query);
...
while ( $row=mysqli_fetch_array($query) ) 	// preparamos array
{
    ...
    $nestedData[] = $row["id"];
    $nestedData[] = $row["tipo"];
    $nestedData[] = $row["idCategoria"];
    $nestedData[] = $row["idCuenta"];
    $nestedData[] = date("d/m/Y", strtotime($row["fecha"]));
    $nestedData[] = $row["importe"];


    $nestedData[] = '<td><center>
							<a href="editIngresosGastos.php?id='.$row['id'].'"  data-toggle="tooltip" title="Editar datos" class="btn btn-sm btn-info"> <i class="fas fa-edit"></i> Editar </a>
							<a href="IngresosGastos.php?action=delete&id='.$row['id'].'"  data-toggle="tooltip" title="Eliminar" class="btn btn-sm btn-danger"> <i class="fa fa-trash"></i> </a>
							</center></td>';
