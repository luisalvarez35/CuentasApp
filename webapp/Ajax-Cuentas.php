...
$sql = "SELECT id, idUsuario, descripcion";
$sql.=" FROM cuentas";
$sql.=" WHERE idUsuario = '".$_SESSION['id']."' ";
$query = mysqli_query($mysqli, $sql) or die("Ajax-Cuentas.php: obtiene los datos para el grid.");
...
if ( !empty($requestData['search']['value']) )
{
    ...
    $sql.=" FROM cuentas";
    $sql.=" WHERE idUsuario = '".$_SESSION['id']."' ";
    $sql.=" AND (descripcion LIKE '".$requestData['search']['value']."%' )";    // $requestData['search']['value'] contiene parametros de busqueda
    $query=mysqli_query($mysqli, $sql) or die("Ajax-Cuentas.php: get PO");
    ...
    $query=mysqli_query($mysqli, $sql) or die("Ajax-Cuentas.php: get PO"); // Ejecutamos la consulta otra vez con los límites indicados
    ////////////////////////////////////////////////////////////////////////////////////////////////
}
...
while ( $row=mysqli_fetch_array($query) ) 	// preparamos array
{
    ...
    $nestedData[] = '<td><center>
							<a href="editCuentas.php?id='.$row['id'].'"  data-toggle="tooltip" title="Editar datos" class="btn btn-sm btn-info"> <i class="fas fa-edit"></i> Editar </a>
							<a href="Cuentas.php?action=delete&id='.$row['id'].'"  data-toggle="tooltip" title="Eliminar" class="btn btn-sm btn-danger"> <i class="fa fa-trash"></i> </a>
							</center></td>';
    $data[] 	  = $nestedData;
}
